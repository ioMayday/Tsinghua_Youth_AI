...
def loadImg(imgPath):
    ...
    for i in range(len(imgPath)):        # 循环，在读入的图片路径中循环
        imageData.append({0})        # 读入图片数据，存入imgData，需要调用函数cv2.imread，采用cv2.IMREAD_GRAYSCALE关键字
    imageData = np.array(imageData)# 将imgData格式修改为array格式
...
# 下载数据
trainImgP,trainlabel = {0}        # 下载训练数据与标记
...
# 增加全连接层，创建网络 
A1 = fully_connected_batchNorm(prev_layer={0}, num_units=1024,is_training=True)  #(0)为待补充,
A2 = fully_connected_batchNorm(prev_layer={0},num_units=2048,is_training=True)  #(0)为待补充
A3 = fully_connected_batchNorm(prev_layer={0},num_units=2048,is_training=True)  #(0)为待补充
A4 = fully_connected_batchNorm(prev_layer={0},num_units=1024,is_training=True)  #(0)为待补充
y  = tf.layers.dense(inputs={0},units=1, activation=None) 
...
def Ttest(testImgPath,testL):        # 测试函数
    ...
    test_pred1 = predict({0})        # 利用predict函数计算预测值
...
def Ttrain(trainImagePath,trainL,seed):        # 训练函数
    ...
    for minibatchT in minibatches:        # 在minibatches中循环
        ...
        [trainPred]=sess.run([y], feed_dict={x: {0}, y_: {0}})        # 计算trainPred
...
for i in range(epochs):        # 循环遍历epochs
    ...
    for j in range(len(minibatches)):        # 循环遍历minibatches
        ...
        newX = minibatch_X.reshape({0})        # 重整图片尺寸
        train_step.run(feed_dict={x: newX, y_: minibatch_Y})        # 将数据放入训练网络
        
    end = time.clock()        # 将当前时间放入end变量
    print('epochs:{0} takes time :{1} s'.format(epoch_cost,({0})))        # 打印结果
...
[cross_entropy_loss,y_pred,lr_out] = sess.run([{0},y,learning_rate],feed_dict={x: newX, y_: minibatch_Y})
