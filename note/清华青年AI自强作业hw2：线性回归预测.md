...
本次作业任务是根据excel中的数据：身高、收入和喜欢程度，设计一个线性回归算法以进行预测（输入身高和收入，输出是否喜欢），并初步熟悉神经网络的基本流程。
...
-   `sigmoid`函数控制输出为0-1，便于与分类标签01区分
-   计算误差，代入到公式中更新权重
-   满足迭代次数后，返回权重
...
附待完成的作业代码如下，读者可以自行尝试填写`请补全`部分，并调试观察效果。
...
**作业效果：**
![在这里插入图片描述](https://img-blog.csdnimg.cn/ac89c62fc8cb43ae95eea44df793deb4.png#pic_center)
